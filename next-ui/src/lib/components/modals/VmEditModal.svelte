<script lang="ts">
	...
	let formData = $state({
		...
		// Clustering
		vm_clustered: vm.clustered || false,
		...
		integration_services_data_exchange:
			vm.integration_services_data_exchange ?? true,
		integration_services_heartbeat:
			vm.integration_services_heartbeat ?? true,
		integration_services_vss_backup:
			vm.integration_services_vss_backup ?? true,
		integration_services_guest_services:
			vm.integration_services_guest_services ?? false,
	});
	...
	let isDirty = $derived(
		formData.cpu_cores !== (vm.cpu_cores || 1) ||
			formData.memory_startup_gb !==
				(vm.memory_startup_gb || vm.memory_gb || 1) ||
			formData.memory_min_gb !== (vm.memory_min_gb || 0.5) ||
			formData.memory_max_gb !== (vm.memory_max_gb || 2) ||
			formData.dynamic_memory_enabled !==
				(vm.dynamic_memory_enabled || false) ||
			formData.memory_prcnt_buffer !== (vm.dynamic_memory_buffer || 20) ||
			formData.secure_boot !==
				(vm.secure_boot_template || "Microsoft Windows") ||
			formData.tpm_enabled !==
				(vm.trusted_platform_module_enabled || false) ||
			formData.vm_clustered !== (vm.clustered || false) ||
			formData.host_recovery_action !==
				(vm.host_recovery_action || "none") ||
			formData.host_stop_action !== (vm.host_stop_action || "save") ||
			formData.integration_services_shutdown !==
				(vm.integration_services_shutdown ?? true) ||
			formData.integration_services_time !==
				(vm.integration_services_time ?? true) ||
			formData.integration_services_data_exchange !==
				(vm.integration_services_data_exchange ?? true) ||
			formData.integration_services_heartbeat !==
				(vm.integration_services_heartbeat ?? true) ||
			formData.integration_services_vss_backup !==
				(vm.integration_services_vss_backup ?? true) ||
			formData.integration_services_guest_services !==
				(vm.integration_services_guest_services ?? false),
	);
	...
	function validate(): boolean {
		...
		if (formData.dynamic_memory_enabled) {
			if (formData.memory_min_gb > formData.memory_startup_gb) {
				validationErrors.memory_min_gb =
					"Minimum memory cannot be greater than startup memory";
			}
			if (formData.memory_max_gb < formData.memory_startup_gb) {
				validationErrors.memory_max_gb =
					"Maximum memory cannot be less than startup memory";
			}
	...
	async function handleSubmit(e: Event) {
		...
		if (formData.cpu_cores !== (vm.cpu_cores || 1))
			patchBody.cpu_cores = formData.cpu_cores;
		if (
			formData.memory_startup_gb !==
			(vm.memory_startup_gb || vm.memory_gb || 1)
		)
			patchBody.memory_startup_gb = formData.memory_startup_gb;
		if (formData.memory_min_gb !== (vm.memory_min_gb || 0.5))
			patchBody.memory_min_gb = formData.memory_min_gb;
		if (formData.memory_max_gb !== (vm.memory_max_gb || 2))
			patchBody.memory_max_gb = formData.memory_max_gb;
		if (
			formData.dynamic_memory_enabled !==
			(vm.dynamic_memory_enabled || false)
		)
			patchBody.dynamic_memory_enabled = formData.dynamic_memory_enabled;
		if (formData.memory_prcnt_buffer !== (vm.dynamic_memory_buffer || 20))
			patchBody.memory_prcnt_buffer = formData.memory_prcnt_buffer;
		if (
			formData.secure_boot !==
			(vm.secure_boot_template || "Microsoft Windows")
		)
			patchBody.secure_boot = formData.secure_boot;
		if (
			formData.tpm_enabled !==
			(vm.trusted_platform_module_enabled || false)
		)
			patchBody.tpm_enabled = formData.tpm_enabled;
		if (formData.vm_clustered !== (vm.clustered || false))
			patchBody.vm_clustered = formData.vm_clustered;
		if (
			formData.host_recovery_action !==
			(vm.host_recovery_action || "none")
		)
			patchBody.host_recovery_action = formData.host_recovery_action;
		if (formData.host_stop_action !== (vm.host_stop_action || "save"))
			patchBody.host_stop_action = formData.host_stop_action;
		if (
			formData.integration_services_shutdown !==
			(vm.integration_services_shutdown ?? true)
		)
			patchBody.integration_services_shutdown =
				formData.integration_services_shutdown;
		if (
			formData.integration_services_time !==
			(vm.integration_services_time ?? true)
		)
			patchBody.integration_services_time =
				formData.integration_services_time;
		if (
			formData.integration_services_data_exchange !==
			(vm.integration_services_data_exchange ?? true)
		)
			patchBody.integration_services_data_exchange =
				formData.integration_services_data_exchange;
		if (
			formData.integration_services_heartbeat !==
			(vm.integration_services_heartbeat ?? true)
		)
			patchBody.integration_services_heartbeat =
				formData.integration_services_heartbeat;
		if (
			formData.integration_services_vss_backup !==
			(vm.integration_services_vss_backup ?? true)
		)
			patchBody.integration_services_vss_backup =
				formData.integration_services_vss_backup;
		if (
			formData.integration_services_guest_services !==
			(vm.integration_services_guest_services ?? false)
		)
			patchBody.integration_services_guest_services =
				formData.integration_services_guest_services;
		...
		try {
			const response = await fetch(
				`/api/v1/virtualmachines/${encodeURIComponent(vm.id)}`,
				{
					method: "PATCH",
...
<Modal title="Edit Virtual Machine" bind:isOpen {onClose} width="lg">
	<div class="flex flex-col h-full">
		...
		<div class="flex border-b border-border mb-4 gap-1">
			{#each tabs as tab}
				<button
					class="px-4 py-2 text-sm font-medium border-b-2 transition-colors hover:bg-surface-2 rounded-t-md {activeTab ===
					tab.id
		...
		<form
			...
			class="space-y-4 flex-1 overflow-y-auto p-1"
		>
			...
			{:else if activeTab === "security"}
				...
				>
					...
					>
						...
						>
						<option value="Microsoft UEFI Certificate Authority"
							>Microsoft UEFI Certificate Authority</option
						>
						<option value="Open Source Shielded VM"
							>Open Source Shielded VM</option
						>
				...
				<div class="grid grid-cols-2 gap-4">
					...
					>
						...
						>
							...
							<option value="resume"
								>Automatically Start if Running</option
							>
					...
					<div class="space-y-2">
						<div class="flex items-center gap-2">
							...
							<label
								...
								>Operating System Shutdown</label
							>
						</div>
						<div class="flex items-center gap-2">
							<input
						...
						<div class="flex items-center gap-2">
							...
							<label
								for="is_backup"
								class="text-sm text-text-primary"
								>Backup (volume shadow copy)</label
							>
						</div>
						<div class="flex items-center gap-2">
							<input
