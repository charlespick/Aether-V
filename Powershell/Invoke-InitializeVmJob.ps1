...
# Main execution with envelope error handling
try {
    ...
    # Return standardized JobResult envelope
    $result = @{
        status  = "success"
        ...
        logs    = @(
            "VM guest initialization orchestration completed",
            "Provisioning ISO mounted and VM started",
            "Guest configuration published via KVP",
            "Guest provisioning completed successfully"
        )
    }
    ...
    # Return error envelope on any failure
    $errorResult = @{
        status  = "error"
        message = "VM guest initialization failed: $($_.Exception.Message)"
        data    = @{
            error_details      = $_.Exception.Message
            script_stack_trace = $_.ScriptStackTrace
        }
        logs    = @(
            "VM guest initialization failed",
            "Error: $($_.Exception.Message)"
        )
    }
    
    $errorJson = $errorResult | ConvertTo-Json -Depth 10 -Compress
