...
function Measure-Step {
    ...
    try {
        ...
        $sw.Stop()
        $script:timings[$Name] = $sw.ElapsedMilliseconds
    }
}
...
try {
    ...
    Measure-Step 'ClusterNode' {
        try {
            $script:clusterNode = Get-ClusterNode -Name $ComputerName -ErrorAction Stop
            if ($script:clusterNode -and $script:clusterNode.Cluster) {
                $result.Host.ClusterName = $script:clusterNode.Cluster.Name
            }
        } catch {
    ...
    $vmDataByGuid = @{}
    $vmGuidToName = @{}
    Measure-Step 'CIM-ComputerSystem' {
        try {
    ...
    if ($vmDataByGuid.Count -eq 0) {
        ...
        if ($ShowTimings) { $result.Timings = $timings }
        $result | ConvertTo-Json -Depth 8
        exit 0
    }

    # Get VM settings (CreationTime, Notes, Version, Generation)
    Measure-Step 'CIM-VirtualSystemSettingData' {
        try {
    ...
    # Get memory settings
    Measure-Step 'CIM-MemorySettingData' {
        try {
    ...
    Measure-Step 'CIM-ProcessorSettingData' {
        try {
    ...
    Measure-Step 'CIM-SyntheticEthernetPortSettings' {
        try {
    ...
    $switchByAdapterId = @{}
    Measure-Step 'CIM-EthernetPortAllocation' {
        try {
    ...
    $ipsByAdapterId = @{}
    Measure-Step 'CIM-GuestNetworkConfig' {
        try {
    ...
    Measure-Step 'Get-VMNetworkAdapter-Index' {
    ...
    $vlanById = @{}
    Measure-Step 'CIM-EthernetSwitchPortVlan' {
        try {
    ...
    Measure-Step 'CIM-StorageAllocation' {
        try {
    ...
    $mergeDiskMs = 0
    $mergeBaseMs = 0
    Measure-Step 'MergeData' {
        $script:vmDetails = foreach ($vmName in $vmDataByName.Keys) {
            ...
            $swBase.Stop()
            $script:mergeBaseMs += $swBase.ElapsedMilliseconds
            ...
            $swNet.Stop()
            $script:mergeNetworkMs += $swNet.ElapsedMilliseconds
            ...
            $swDisk.Stop()
            $script:mergeDiskMs += $swDisk.ElapsedMilliseconds

            # Output the VM info
            $vmInfo
        }
    }
    $timings['MergeData-Base'] = $mergeBaseMs
    $timings['MergeData-Networks'] = $mergeNetworkMs
    $timings['MergeData-Disks'] = $mergeDiskMs

    $result.VirtualMachines = @($vmDetails)

    # Load host resources configuration and convert VLAN numbers to network names
    Measure-Step 'NetworkNameResolution' {
        try {
    ...
    # Add timings to output if requested
    if ($ShowTimings) {
