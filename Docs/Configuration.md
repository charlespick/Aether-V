...
| `WINRM_KERBEROS_PRINCIPAL`      | _(unset)_                    | No                      | Kerberos principal for the service account UPN used to generate the keytab (e.g., `svc-aetherv@AD.EXAMPLE.COM`). Required for Hyper-V host management. If you prefer a service SPN such as `HTTP/aetherv.example.com@AD.EXAMPLE.COM`, ensure that SPN is registered to the same account and present in the keytab. |
...
| `WINRM_KEYTAB_B64`         | Base64-encoded Kerberos keytab for WinRM access. | Required for Hyper-V host management. Generate with `base64 < service.keytab \| tr -d '\n'` and set as single-line value.                |
