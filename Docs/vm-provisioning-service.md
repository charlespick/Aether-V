...
- `/api/v1/schema/job-inputs` exposes the currently active schema so the UI can
  ...
  upgrades.【F:server/app/api/routes.py†L344-L373】【F:server/app/core/models.py†L156-L191】
- The schema lives in `Schemas/job-inputs.yaml` and defines field metadata,
  ...
  Windows-specific domain join fields) without hard-coding logic.【F:Schemas/job-inputs.yaml†L1-L141】
- `validate_job_submission` normalises user input, applies cross-field
  ...
  validation errors before any remote work is attempted.【F:server/app/core/job_schema.py†L40-L211】
...
- Provisioning requests call `submit_provisioning_job`, which re-checks the
  ...
  while the agent deployment service reports incomplete rollouts.【F:server/app/api/routes.py†L368-L432】
- Each accepted request is converted into a portable job definition containing
  ...
  contract.【F:server/app/api/routes.py†L424-L434】
...
- `JobService.start()` initialises an asyncio queue and launches the configured
  ...
  publishes notifications, and enqueues the job ID for background execution.【F:server/app/services/job_service.py†L203-L287】
- Worker loops transition jobs into the running state, wrap execution in
  ...
  so downstream listeners receive consistent lifecycle events.【F:server/app/services/job_service.py†L288-L455】【F:server/app/services/job_service.py†L612-L672】
- `get_metrics` summarises queue depth, worker count, and job status tallies for
  ...
  stuck work.【F:server/app/services/job_service.py†L688-L717】
...
- `_execute_provisioning_job` waits for `ensure_host_setup` to finish preparing
  ...
  the event loop.【F:server/app/services/job_service.py†L360-L413】
- `RemoteTaskService` multiplexes blocking tasks across separate fast and job
  ...
  while keeping long-running provisioning scripts isolated from lighter work.【F:server/app/services/remote_task_service.py†L1-L117】
- Streaming callbacks feed stdout/stderr chunks into a `_PowerShellStreamDecoder`
  ...
  receive clean, incremental log lines over websockets and notifications.【F:server/app/services/job_service.py†L24-L196】【F:server/app/services/job_service.py†L414-L571】
...
- Job updates drive persistent notifications, websocket broadcasts (per-job and
  ...
  show accurate state even before Hyper-V reports the new machine.【F:server/app/services/job_service.py†L521-L603】【F:server/app/services/inventory_service.py†L1248-L1282】
- On status transitions the service publishes summaries via the notification
  ...
  audit history aligned with actual provisioning outcomes.【F:server/app/services/job_service.py†L214-L252】【F:server/app/services/job_service.py†L603-L672】
...
- The job definition stored with each request mirrors the schema and cleaned
  ...
  it into `Invoke-ProvisioningJob.ps1` on the host.【F:server/app/services/job_service.py†L552-L606】【F:server/app/services/job_service.py†L643-L676】
- Within the script, `Read-ProvisioningJobDefinition` accepts JSON from standard
  ...
  field identifiers supplied by the API.【F:Powershell/Invoke-ProvisioningJob.ps1†L60-L109】
- Because both sides rely on the same schema identifiers and versioning, frontend
  ...
  jobs.【F:server/app/api/routes.py†L368-L396】【F:server/app/core/job_schema.py†L40-L211】
