...
@router.get("/auth/ws-token", tags=["Authentication"])
async def get_websocket_token(request: Request, user: dict = Depends(get_current_user)):
    """
    Get a temporary token for WebSocket authentication.
    
    This endpoint generates a short-lived token that can be used to authenticate
    WebSocket connections. The token is valid for 5 minutes.
    """
    import jwt as pyjwt
    import time
    
    # Generate a simple JWT token for WebSocket authentication
    # This token will be validated by the WebSocket endpoint
    token_data = {
        "sub": user.get("sub"),
        "preferred_username": user.get("preferred_username", user.get("sub")),
        "roles": user.get("roles", []),
        "auth_type": user.get("auth_type", "session"),
        "exp": int(time.time()) + 300,  # 5 minute expiry
        "iat": int(time.time()),
        "iss": "aether-v-orchestrator",
        "aud": "websocket"
    }
    
    # Use session secret or generate a temporary one
    secret = settings.session_secret_key or "temporary-websocket-secret"
...
async def authenticate_websocket(websocket: WebSocket) -> Optional[dict]:
    """
    Authenticate WebSocket connection using the same strategy as REST API.
    
    Supports:
    - WebSocket-specific tokens (from /auth/ws-token endpoint)
    - OIDC JWT tokens
    - Static API token
    - Development mode (if enabled)
    ...
    if not settings.auth_enabled:
        if not settings.allow_dev_auth:
            logger.error(f"Auth disabled but ALLOW_DEV_AUTH not set - WebSocket connection denied from {client_ip}")
            return None
    ...
    # Try token from query parameter (standard for WebSocket authentication)
    token = websocket.query_params.get("token")
    
    if not token:
        logger.warning(f"WebSocket authentication failed: no token provided from {client_ip}")
        return None
    ...
    # Try WebSocket-specific token (generated by /auth/ws-token endpoint)
    try:
        import jwt as pyjwt
        ...
        payload = pyjwt.decode(token, secret, algorithms=["HS256"], audience="websocket")
        
        # Check if it's a WebSocket token (has correct audience)
        if payload.get("iss") == "aether-v-orchestrator" and payload.get("aud") == "websocket":
            logger.info(f"WebSocket token authentication successful for {payload.get('preferred_username', 'unknown')} from {client_ip}")
            return payload
            
    except pyjwt.ExpiredSignatureError:
        logger.warning(f"WebSocket token expired from {client_ip}")
    except pyjwt.InvalidTokenError as e:
        logger.debug(f"WebSocket token validation failed (not a WS token, will try OIDC): {e}")
    except Exception as e:
        logger.debug(f"Error validating WebSocket token from {client_ip}: {e}")
        ...
        try:
            ...
            if settings.oidc_role_name and settings.oidc_role_name != "*":
                if settings.oidc_role_name not in user_roles:
                    ...
                    logger.warning(f"WebSocket user {username} from {client_ip} lacks required role '{settings.oidc_role_name}'")
                    return None
    ...
    logger.warning(f"WebSocket authentication failed: invalid token from {client_ip}")
    return None
